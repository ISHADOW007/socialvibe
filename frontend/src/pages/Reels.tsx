...
export default function Reels() {
        ...
        entries.forEach((entry) => {
          if (entry.isIntersecting) {
            const index = parseInt(
              entry.target.getAttribute("data-reel-index") || "0"
            );
  ...
  const handleAddComment = async (reelId: string, text: string) => {
    try {
      const { comment, commentsCount } = await reelService.addComment(
        reelId,
        text
      );
      setReels((prev) =>
        ...
        <div className="h-screen flex flex-col items-center justify-center bg-black text-white px-6">
          <div className="text-center">
            ...
            <p className="text-gray-400 mb-8 max-w-sm">
              Be the first to share a reel! Create engaging short videos to
              connect with others.
            </p>
            {user && (
              <Button
